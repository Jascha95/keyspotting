...
def open_specific_device_in_sequence(device_id : int):
    ...
    opened_devices = {}
    print(f"Opening device #{device_id} (approx. {0})...".format(time.strftime("%M min %S sec", time.gmtime(102*device_id))))
...
def save_param_group_to_txt_file(config, chip_id : int, core_id : int, filename : str):
    """Save the parameters of a specific core to the file
    ...
    # params = parameter_group.param_map.values()
    
    params = get_parameters(config, chip_id, core_id)
    for param in params:
        save_file.write(
            '{0}:\tidx={1},\ttuple=({2},{3}),\tlinear={4} \t # {5}\n'.format(
                param.param_name,
                _bias_as_idx((param.coarse_value, param.fine_value),'tuple'),
                param.coarse_value,
                param.fine_value,
                _bias_as_linear((param.coarse_value, param.fine_value),'tuple'),
                BIAS_NAMES_FULL[BIAS_NAMES_LOOKUP[param.param_name]]))
        
    save_file.close()
        
        
def set_param_group_from_txt_file(model, chip_id : int, core_id : int, filename : str, priority_bias_format="tuple"):
    """Load the parameters onto a single core, with attention to only one specified parameter format in case of conflicts.
    
    Parameters
    ----------
        model (Dynapse1Model): device model
        chip_id (int): target chip ID
        core_id (int): target core ID
        filename (str): TXT file with saved parameters
        priority_bias_format (str): can be "tuple", "linear" or "index" 
    """
    
    with open(filename) as f:
        lines = f.read().splitlines()

    non_empty_lines = [line.split('\n')[0].split('#')[0].strip() for line in lines if line.strip() != ""]

    # Strips the newline character
    for line in non_empty_lines:
        name = line.split(':')[0].strip()
        if name in BIAS_NAMES:
            bias_idx = int(line.split('idx=')[1].split(',')[0].strip())
            coarse_value = int(line.split('tuple=(')[1].split(',')[0].strip())
            fine_value = int(line.split('tuple=(')[1].split(',')[1].split(')')[0].strip())
            linear_value = int(line.split('linear=')[1].split(' ')[0].split('#')[0].strip())
            
            if not _bias_as_idx(bias_idx, 'index') == _bias_as_idx((coarse_value, fine_value), 'tuple') == _bias_as_idx(linear_value, 'linear'):
                print(f"PARAMETER WARNING: inconsistency between values given for parameter \'{BIAS_NAMES_FULL[BIAS_NAMES_LOOKUP[name]]}\'; \'{priority_bias_format}\' value is applied")
                print("idx={0},\ttuple=({1},{2}),\tlinear={3}".format(bias_idx, coarse_value, fine_value, linear_value))
                
            if priority_bias_format=="index":
                coarse_value, fine_value = _bias_as_tuple(bias_idx, 'index')
            elif priority_bias_format=="linear":
                coarse_value, fine_value = _bias_as_tuple(linear_value, 'linear')
            else:
                coarse_value, fine_value = _bias_as_tuple((coarse_value, fine_value), 'tuple')

            param = dyn1.Dynapse1Parameter(name, int(coarse_value), int(fine_value))

            model.update_single_parameter(param, int(chip_id), int(core_id))
...
def get_bias_index_from_linear_bias(linear : int):
...
def get_coarse_and_fine_from_linear_bias(linear : int):
    ...
    idx = get_bias_index_from_linear_bias(linear)
    
    return (LINEAR_BIAS_MAP[idx,0], LINEAR_BIAS_MAP[idx,1])
    
    
def set_bias_by_index(idx : int):
    """ Set coarse and fine values of the parameter from the id in the parameter map
...
def set_linear_bias_by_index(idx : int):
    """ Set linear value of the parameter from the id in the parameter map
...
def get_linear_bias(coarse : int, fine : int):
    ...
    """
    return int(100*get_linear_bias_current(coarse, fine))


def copy_parameters2core(model, source_core : int, target_core : int):
    """Copy all parameters from the source core to the target.
    
    Args:
        model (Dynapse1Model): the model of the device
        source_core (int)[0..15] : source core id to copy parameters from, without chip id
        target_core (int)[0..15] : target core id to set parameters on, without chip id
    """
...
def validate_bias_tuple(bias_value):
    
    coarse_value, fine_value = bias_value
    assert coarse_value < 8 and coarse_value >=0, "ERROR: Invalid coarse value in a parameter tuple"
    assert fine_value < 256 and fine_value >=0, "ERROR: Invalid fine value in a parameter tuple"
    
    coarse_value, fine_value = get_coarse_and_fine_from_linear_bias(get_linear_bias(coarse_value, fine_value))
    
    return (coarse_value, fine_value)
    
    
def parse_bias_value(bias_value, bias_format=None):
    """Parsing and validation of the input bias value. The function checks if the given value is within the bounds for the given
       format, or tries to guess the best format match. In case of inconsistencies prints warnings.

    Args:
        bias_value (int or tuple): bias value to validate
        bias_format (str, optional): can be "tuple", "index" or "linear". Defaults to None.
    ...
    Returns:
        (valid_bias_value, bias_format): valid bias values is returned together with the corresponding format.
    """
    
    if type(bias_value)==tuple:
        coarse_value, fine_value = validate_bias_tuple(bias_value)
        if bias_format != None and bias_format != "tuple":
            print(f"WARNING: Parameter value is parsed as a valid (Coarse, Fine) tuple, but the given bias_format \'{bias_format}\' is mismatching")
        return (coarse_value, fine_value), "tuple"
        
    if type(bias_value)==int:
        if bias_value >= 0:
            if bias_value<=2.4e+09:
                if bias_value>=1812:
                    if bias_format == "index":
                        raise ValueError("ERROR: Parameter parsed as Index exceeds the maximum value")
                    if bias_format != None and bias_format != "linear":
                        print(f"WARNING: Parameter is parsed as a valid Linear value, but the given bias_format \'{bias_format}\' is mismatching")
                    return bias_value, "linear"
                elif bias_format != "index":
                    if bias_format != "linear":
                        print(f"WARNING: Ambiguous parameter value encountered ({bias_value} could be both \'index\' or \'linear\'). The value will be parsed as Linear by default. Please specify \"index\" to use that type instead.")
                    return bias_value, "linear"
                else:
                    return bias_value, "index"
                        
            else:
                raise ValueError(f"ERROR: Parameter parsed as Linear exceeds the maximum value ({bias_value})")
        else:
            raise ValueError("ERROR: Parameter can't be negative")
    
    raise ValueError("ERROR: Invalid parameter/bias type. Possible types for bias_value are \'tuple\' or \'int\'; possible bias_format are \'tuple\', \'index\' or \'linear\'")

def _bias_as_tuple(bias_value, bias_format):
    """Conversion function of any bias format given to the type "tuple" (i.e. coarse and fine)

    Args:
        bias_value (tuple or int): bias value
        bias_format (str): bias format

    Returns:
        (coarse, fine): coarse and fine parameter tuple
    """
    
    bias_value, bias_format = parse_bias_value(bias_value, bias_format)
    if bias_format == "tuple":
        coarse_value, fine_value = bias_value
    elif bias_format == "index":
        coarse_value, fine_value = set_bias_by_index(bias_value)
    else:
        coarse_value, fine_value = get_coarse_and_fine_from_linear_bias(bias_value)
    return int(coarse_value), int(fine_value)

def _bias_as_linear(bias_value, bias_format):
    """Conversion function of any bias format given to the type "linear"
    ...
    Returns:
        linear (int): linear parameter value
    """
    
    bias_value, bias_format = parse_bias_value(bias_value, bias_format)
    if bias_format == "linear":
        linear_value = set_linear_bias_by_index(get_bias_index_from_linear_bias(bias_value))
    elif bias_format == "index":
        linear_value = set_linear_bias_by_index(bias_value)
    else:
        linear_value = get_linear_bias(bias_value[0], bias_value[1])
    return int(linear_value)

def _bias_as_idx(bias_value, bias_format):
    
    bias_value, bias_format = parse_bias_value(bias_value, bias_format)
    if bias_format == "index":
        index_value = bias_value
    elif bias_format == "linear":
        index_value = get_bias_index_from_linear_bias(bias_value)
    else:
        index_value = get_bias_index_from_linear_bias(get_linear_bias(bias_value[0], bias_value[1]))
    return int(index_value)

    
def _set_bias(model, chip_id : int, core_id : int, bias_name : str, bias_value, bias_format=None):
    
    coarse_value, fine_value = _bias_as_tuple(bias_value, bias_format)
    
    param = dyn1.Dynapse1Parameter(bias_name, coarse_value, fine_value)
    model.update_single_parameter(param, chip_id, core_id)
    
    
def _get_bias(model, chip_id : int, core_id : int, bias_name : str, bias_format=None):
    
    configuration = model.get_configuration()
    fine = int(configuration.chips[chip_id].cores[core_id].parameter_group.param_map[bias_name].fine_value)
    coarse = int(configuration.chips[chip_id].cores[core_id].parameter_group.param_map[bias_name].coarse_value)
    linear = int(configuration.chips[chip_id].cores[core_id].parameter_group.get_linear_parameter(bias_name))
    index = int(get_bias_index_from_linear_bias(linear))
...
def get_neuron_tau1(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_TAU1_N", bias_format=bias_format)
...
def get_neuron_tau2(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_TAU2_N", bias_format=bias_format)
...
def get_neuron_gain(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_THR_N", bias_format=bias_format)
...
def get_neuron_refractory_period(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_RFR_N", bias_format=bias_format)
...
def get_neuron_dc(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_DC_P", bias_format=bias_format)

def set_neuron_adaptation_tau(model, chip_id : int, core_id : int, bias_value, bias_format=None, fixed_gain_ratio=None):
    _set_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_AHTAU_N", bias_value=bias_value, bias_format=bias_format)
    
    if fixed_gain_ratio:
        if (type(fixed_gain_ratio)==int or type(fixed_gain_ratio)==float) and fixed_gain_ratio>0:
            bias_value_linear = _bias_as_linear(bias_value, bias_format)
            gain_bias_value = int(np.clip(fixed_gain_ratio*bias_value_linear, LINEAR_BIAS_MIN, LINEAR_BIAS_MAX))
            set_neuron_adaptation_gain(model, chip_id, core_id, gain_bias_value, 'linear')
        else:
            raise ValueError("Invalid gain ratio, AHP gain not set")

def get_neuron_adaptation_tau(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_AHTAU_N", bias_format=bias_format)
...
def get_neuron_adaptation_weight(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_AHW_P", bias_format=bias_format)
...
def get_neuron_adaptation_gain(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_AHTHR_N", bias_format=bias_format)
...
def get_adaptation_enable(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_CASC_N", bias_format=bias_format)
...
def get_ampa_weight(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="PS_WEIGHT_EXC_F_N", bias_format=bias_format)
...
def get_ampa_gain(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPIE_THR_F_P", bias_format=bias_format)

def set_ampa_tau(model, chip_id : int, core_id : int, bias_value, bias_format=None, fixed_gain_ratio=None):
    ...
    if fixed_gain_ratio:
        if (type(fixed_gain_ratio)==int or type(fixed_gain_ratio)==float) and fixed_gain_ratio>0:
            bias_value_linear = _bias_as_linear(bias_value, bias_format)
            gain_bias_value = int(np.clip(fixed_gain_ratio*bias_value_linear, LINEAR_BIAS_MIN, LINEAR_BIAS_MAX))
            set_ampa_gain(model, chip_id, core_id, gain_bias_value, 'linear')
        else:
            raise ValueError("Invalid gain ratio, AMPA gain not set")

def get_ampa_tau(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPIE_TAU_F_P", bias_format=bias_format)
...
def get_nmda_weight(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="PS_WEIGHT_EXC_S_N", bias_format=bias_format)
...
def get_nmda_gain(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPIE_THR_S_P", bias_format=bias_format)

def set_nmda_tau(model, chip_id : int, core_id : int, bias_value, bias_format=None, fixed_gain_ratio=None):
    ...
    if fixed_gain_ratio:
        if (type(fixed_gain_ratio)==int or type(fixed_gain_ratio)==float) and fixed_gain_ratio>0:
            bias_value_linear = _bias_as_linear(bias_value, bias_format)
            gain_bias_value = int(np.clip(fixed_gain_ratio*bias_value_linear, LINEAR_BIAS_MIN, LINEAR_BIAS_MAX))
            set_nmda_gain(model, chip_id, core_id, gain_bias_value, 'linear')
        else:
            raise ValueError("Invalid gain ratio, NMDA gain not set")

def get_nmda_tau(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPIE_TAU_S_P", bias_format=bias_format)
...
def get_nmda_enable(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_NMDA_N", bias_format=bias_format)
...
def get_gaba_a_weight(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="PS_WEIGHT_INH_F_N", bias_format=bias_format)
...
def get_gaba_a_gain(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPII_THR_F_P", bias_format=bias_format)

def set_gaba_a_tau(model, chip_id : int, core_id : int, bias_value, bias_format=None, fixed_gain_ratio=None):
    ...
    if fixed_gain_ratio:
        if (type(fixed_gain_ratio)==int or type(fixed_gain_ratio)==float) and fixed_gain_ratio>0:
            bias_value_linear = _bias_as_linear(bias_value, bias_format)
            gain_bias_value = int(np.clip(fixed_gain_ratio*bias_value_linear, LINEAR_BIAS_MIN, LINEAR_BIAS_MAX))
            set_gaba_a_gain(model, chip_id, core_id, gain_bias_value, 'linear')
        else:
            raise ValueError("Invalid gain ratio, GABA A gain not set")

def get_gaba_a_tau(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPII_TAU_F_P", bias_format=bias_format)
...
def get_gaba_b_weight(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="PS_WEIGHT_INH_S_N", bias_format=bias_format)
...
def get_gaba_b_gain(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPII_THR_S_P", bias_format=bias_format)

def set_gaba_b_tau(model, chip_id : int, core_id : int, bias_value, bias_format=None, fixed_gain_ratio=None):
    ...
    if fixed_gain_ratio:
        if (type(fixed_gain_ratio)==int or type(fixed_gain_ratio)==float) and fixed_gain_ratio>0:
            bias_value_linear = _bias_as_linear(bias_value, bias_format)
            gain_bias_value = int(np.clip(fixed_gain_ratio*bias_value_linear, LINEAR_BIAS_MIN, LINEAR_BIAS_MAX))
            set_gaba_b_gain(model, chip_id, core_id, gain_bias_value, 'linear')
        else:
            raise ValueError("Invalid gain ratio, GABA A gain not set")

def get_gaba_b_tau(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="NPDPII_TAU_S_P", bias_format=bias_format)
...
def get_pulse_width(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="PULSE_PWLK_P", bias_format=bias_format)
...
def get_r2r_bias(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="R2R_P", bias_format=bias_format)
...
def get_buf_bias(model, chip_id : int, core_id : int, bias_format=None):
    return _get_bias(model, chip_id=chip_id, core_id=core_id, bias_name="IF_BUF_P", bias_format=bias_format)

        
def monitor_neuron(model, chip_id, core_id, neuron_id):
    #TODO: Add input checks
    api  =  model.get_dynapse1_api()
    api.monitor_neuron(chip_id, NEURONS_PER_CORE*core_id + neuron_id)
    
    
    
def dc_steps(model, chip_id, core_id, period, duration, bias_value_max, bias_format='index'):
	...
	Args:
	    ...
	    bias_value_max (int or tuple): amplitude of DC steps
        bias_format (str): the format of the given bias ('index', 'linear' or 'tuple')
     
    """

    start_time = time.time()
    while (time.time() - start_time < duration):
        set_neuron_dc(model, chip_id, core_id, bias_value_max, bias_format)
        time.sleep(period)
        set_neuron_dc(model, chip_id, core_id, 0, 'index')
        time.sleep(period)
