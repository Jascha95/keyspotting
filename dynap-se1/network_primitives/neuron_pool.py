...
class InteractiveNeuronPool:
    ...
    def __init__(self, model, chip_id, core_id, start_neuron, size, num_inp_neurons=None, net_gen=None, inp_matrix=None, matrix=None):
        ...
        if num_inp_neurons:
            ...
            self.input_neurons_exc=[Neuron(0, (core_id+1)%4, (256-size-start_neuron)+n_id, is_spike_gen=True) for n_id in range(self.start_neuron, self.start_neuron+self.size)]
            self.input_neurons_inh=[Neuron(0, (core_id+1)%4, (256-size*2-start_neuron)+n_id, is_spike_gen=True) for n_id in range(self.start_neuron, self.start_neuron+self.size)]
